...
function action_alt_reboot()
  local part = luci.util.trim(luci.sys.exec("/usr/sbin/fw_printenv -n boot_part"))
  local boot = luci.util.trim(luci.sys.exec("/usr/sbin/fw_printenv -n bootcmd"))
  if part == "1" then luci.sys.call("/usr/sbin/fw_setenv boot_part 2") end
  if part == "2" then luci.sys.call("/usr/sbin/fw_setenv boot_part 1") end
